const input = `5-10 b: bhbjlkbbbbbbb
3-4 j: hjvj
8-9 p: pmljtsttp
...
let inputModifier = policy => {
  return {
    passwordStr: policy.match(/[a-z]{2,}/)[0],
    policyObj: {
      letter: policy.match(/[a-z]:/)[0].replace(":", ""),
      lowest: parseInt(policy.match(/\d+-/)[0].replace("-", "")),
      highest: parseInt(policy.match(/-\d+/)[0].replace("-", ""))
    }
  }
}

// First part
function passwordValidity1(password, policy) {
  let count = 0
  password.split("").forEach(letter => {
    if (letter === policy.letter) count++
  })

  return count >= policy.lowest && count <= policy.highest ? true : false
}

function correctPasswordCount1(passwords) {
  let count = 0
  passwords.forEach(item => {
    let modifiedInput = inputModifier(item)
    if (passwordValidity1(modifiedInput.passwordStr, modifiedInput.policyObj)) count++
  })
...
function passwordValidity2 (password, policy) {
  let valid = false
  let lowestValidity = false
  let highestValidity = false

  for (let i = 0; i < password.length; i++) {
    if (i + 1 === policy.lowest && password[i] === policy.letter) lowestValidity = true
    if (i + 1 === policy.highest && password[i] === policy.letter) highestValidity = true
  }

  if (lowestValidity && highestValidity) {
    valid = false
  } else if (lowestValidity || highestValidity) {
    valid = true
  }
  return valid
}

function correctPasswordCount2(passwords) {
  let count = 0
  passwords.forEach(item => {
    let modifiedInput = inputModifier(item)
    if (passwordValidity2(modifiedInput.passwordStr, modifiedInput.policyObj)) count++
  })

  return count
}

console.log(correctPasswordCount1(inputArray))
console.log(correctPasswordCount2(inputArray))
